...
Marines = function() game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("SetTeam","Marines") end
Pirates = function() game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("SetTeam","Pirates") end
...
Attack.Kill = function(model,Succes)
  if model and Succes then
    ...
    if ToolTip == "Blox Fruit" then 
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,7,0) * CFrame.Angles(0,math.rad(90),0)) 
    else 
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,7,0) * CFrame.Angles(0,math.rad(180),0))
    end
    if RandomCFrame then 
      wait(.5)
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 7, 25)) 
      wait(.5)
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(25, 7, 0)) 
      wait(.5)
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(-25, 7, 0)) 
      wait(.5)
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 7, 25)) 
...
Attack.Kill2 = function(model,Succes)
  if model and Succes then
    ...
    if ToolTip == "Blox Fruit" then 
      ...
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,7,8) * CFrame.Angles(0,math.rad(180),0))
    end
    if RandomCFrame then 
      wait(0.1)
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 7, 25)) 
...
statsSetings = function(Num, value)
  if Num == "Melee" then
    if plr.Data.Points.Value ~= 0 then
      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("AddPoint","Melee",value)
    end
  elseif Num == "Defense" then
    if plr.Data.Points.Value ~= 0 then
      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("AddPoint","Defense",value)
    end
  elseif Num == "Sword" then
    if plr.Data.Points.Value ~= 0 then
      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("AddPoint","Sword",value)
    end
  elseif Num == "Gun" then
    if plr.Data.Points.Value ~= 0 then
      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("AddPoint","Gun",value)
    end
  elseif Num == "Devil" then
    if plr.Data.Points.Value ~= 0 then
      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("AddPoint","Demon Fruit",value)
    end
  end
end
...
UpdStFruit = function()
  ...
  StoreFruit = x:FindFirstChild("EatRemote", true)
    if StoreFruit then
      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("StoreFruit",StoreFruit.Parent:GetAttribute("OriginalName"),
...
GetIn = function(Name)
  for _ ,v1 in pairs(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("getInventory")) do
...
GetM = function(Name)
  for _,tab in pairs(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("getInventory")) do
...
GetWP = function(nametool)
  for _,v4 in pairs(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("getInventory")) do
...
Quests = require(game:GetService("ReplicatedStorage"):WaitForChild("Quests"))
GuideModule = require(game:GetService("ReplicatedStorage"):WaitForChild("GuideModule"))
                ...
                elseif inSub or level < 2600 then
                    ...
                    if not questUI.Visible then
                        ...
                        if questPos then
                            ...
                            if (Root.Position - questPos.Position).Magnitude <= 10 then
                                pcall(function()
                                    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("StartQuest", questData[3], questData[2])
                                end)
                                task.wait(1)
                            end
                        else
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == enemyName and Attack.Alive(v) then
                            ...
                            repeat
                                task.wait(Sec)
                                _tp(v.HumanoidRootPart.CFrame * CFrame.new(0,7,0))
...
-- Functions moved from the UI chunk.
function ModuleTimeZone()
...
local function tweenTo(cframe, duration)
    ...
    -- Gọi hàm tween có sẵn
    if _G._tween then
        _G._tween(cframe)
    elseif _G.Tween then
        _G.Tween(cframe)
    elseif _G.TweenTo then
        _G.TweenTo(cframe)
    else
        _tp(cframe)
    end
    -- Chờ nhân vật đến gần vị trí (15 studs) hoặc hết thời gian
    local root = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if root then
        local startTime = tick()
        while (root.Position - cframe.Position).Magnitude > 15 and tick() - startTime < duration + 1 do
            task.wait(0.1)
        end
    end
end
...
-- UI callback handlers moved out of the UI definition file.
function ModuleWeaponDropdown(Value)
    _G.ChooseWP = Value
end

function ModuleUI2(Value)
        local scales = {Small = 0.8, Normal = 1.0, Big = 1.2}
        Window:SetUIScale(scales[Value])
    end

function ModuleFarmLevel(Value)
        _G.Level = Value
        if not Value then
            alreadyTeleported = false
            teleporting = false
...
function ModuleClosetMons(Value)
  _G.AutoFarmNear = Value
end

function ModuleFactoryRaids(Value)
  _G.AutoFactory = Value
end

function ModuleCastleRaids(Value)
  _G.AutoRaidCastle = Value
end

function ModuleEcto(Value)
  _G.AutoEctoplasm = Value
end

function ModuleChestTW(Value)
  _G.AutoFarmChest = Value
end

function ModuleChestBP(Value)
...
function ModuleWeaponDropdown0(Value)
    _G.StopWhenChalice = Value
end

function ModuleWeaponDropdown1(Value)
  _G.AutoBerry = Value
end

function ModuleWeaponDropdown2(Value)
  _G.AutoBerryH = Value
end

function ModuleWeaponDropdown3(Value)
            getgenv().SelectMob = Value
        end

function ModuleWeaponDropdown4(Value)
            getgenv().SelectMob = Value
        end

function ModuleWeaponDropdown5(Value)
            getgenv().SelectMob = Value
        end

function ModuleWeaponDropdown6(Value)
        _G.AutoKillMob = Value
    end

function ModuleWeaponDropdown7(Value)
            _G.SelectIsland = Value
        end

function ModuleWeaponDropdown8(Value)
            _G.SelectIsland = Value
        end

function ModuleWeaponDropdown9(Value)
            _G.SelectIsland = Value
        end

function ModuleUI20(Value)
        _G.AutoFarmIsland = Value
    end

function ModuleUI21(Value)
    _G.FarmEliteHunt = Value
end

function ModuleUI22(Value)
	_G.FarmEliteH = Value
end

function ModuleUI23(Value)
  _G.AutoRipIngay = Value
end

function ModuleUI24(Value)
  _G.AutoUnHaki = Value
end

function ModuleUI25(Value)
    _G.Auto_Cake_Prince = Value
end

function ModuleUI26(Value)
  _G.AcceptQuestC = Value
end

function ModuleUI27(Value)
    _G.AutoSpawnCP = Value
end

function ModuleUI28(Value)
        _G.AutoDoughKing = Value
    end

function ModuleUI29(Value)
        _G.AutoAttackDoughKing = Value
    end

function ModuleFarmLevel0(Value)
        _G.AutoHop_Dough = Value
    end

function ModuleFarmLevel1(Value)
        _G.AutoFarm_Bone = Value
    end

function ModuleFarmLevel2(Value)
  _G.AcceptQuestB = Value
end

function ModuleFarmLevel3(Value)
  _G.AutoHytHallow = Value
end

function ModuleFarmLevel4(Value)
  _G.Auto_Random_Bone = Value
end

function ModuleFarmLevel5(Value)
  _G.TryLucky = Value
end

function ModuleFarmLevel6(Value)
  _G.Praying = Value
end

function ModuleFarmLevel7(Value) 
    _G.FarmTyrant = Value 
end

function ModuleFarmLevel8(Value)
    _G.FarmPhaBinh = Value
end

function ModuleFarmLevel9(Value)
			getgenv().SelectMaterial = Value
		end

function ModuleClosetMons0(Value)
    getgenv().AutoMaterial = Value
end

function ModuleClosetMons1(value)
			_G.FindBoss = value
		end

function ModuleClosetMons2(value)
        _G.FarmBoss = value
        spawn(function()
            while wait(Sec) do
                if _G.FarmBoss then
                        ...
                        if _G.AcceptQuestBoss and HasQuest then
                            ...
                            if plr.PlayerGui.Main.Quest.Visible == false then
                                ...
                                if (Root.Position - QuestBeta()[5].Position).Magnitude <= 5 then
                                    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("StartQuest", QuestBeta()[3], QuestBeta()[2])
                                end
...
function ModuleClosetMons3(Value)
        _G.AcceptQuestBoss = Value
    end

function ModuleClosetMons4(Value)
    _G.AutoFarmAllBoss = Value
end

function ModuleClosetMons5(Value)
  SelectIsland = Value
end

function ModuleClosetMons6(Value)
  _G.FarmMastery_Dev = Value
end

function ModuleClosetMons7(Value)
  _G.FarmMastery_G = Value
end

function ModuleClosetMons8(Value)
  _G.FarmMastery_S = Value
end

function ModuleClosetMons9(Value)
  _G.Seriality = Value
end

function ModuleFactoryRaids0(Value)
  _B = Value
end

function ModuleFactoryRaids1(Value)
...
function ModuleFactoryRaids2(Value)
        _G.HopDelay = Value * 60
    end

function ModuleFactoryRaids3(Value)
        getgenv().Set = Value
        if Value then
            pcall(function()
...
function ModuleFactoryRaids4(Value)
  Boud = Value
end

function ModuleFactoryRaids5(Value)
        getgenv().Observation = Value
    end

function ModuleFactoryRaids6(Value)
  _G.RaceClickAutov3 = Value
end

function ModuleFactoryRaids7(Value)
  _G.RaceClickAutov4 = Value
end

function ModuleFactoryRaids8(Value)
  RandomCFrame = Value
end

function ModuleFactoryRaids9(Value)
  _G.Safemode = Value
end

function ModuleCastleRaids0(Value)
        _G.DestroyHit = Value
    end

function ModuleCastleRaids1(Value)
  RDeath = Value
end

function ModuleCastleRaids2(Value)
  RemoveDamage = Value
end

function ModuleCastleRaids3(Value)
        if Value then
...
function ModuleCastleRaids4(Value)
        getgenv().HopServerAdmin = Value
    end

function ModuleCastleRaids5(Value)
        getgenv().NoClip = Value
    end

function ModuleCastleRaids6(Value)
  pSats = Value
end

function ModuleCastleRaids7(Value)
  _G.Auto_Melee = Value
end

function ModuleCastleRaids8(Value)
  _G.Auto_Sword = Value
end

function ModuleCastleRaids9(Value)
  _G.Auto_Gun = Value
end

function ModuleEcto0(Value)
  _G.Auto_DevilFruit = Value
end

function ModuleEcto1(Value)
  _G.Auto_Defense = Value
end

function ModuleEcto2(Value)
        _G.SelectedRod = Value
    end

function ModuleEcto3(Value)
...
function ModuleEcto5(Value)
        _G.AutoFishing = Value
    end

function ModuleEcto6(Value)
    _G.AutoFishingQuest = Value
end

function ModuleEcto7(Value)
...
function ModuleEcto9(Value)
    _G.AutoSkillZ = Value
end

function ModuleChestTW0(Value)
  _G.TravelDres = Value
end

function ModuleChestTW1(Value)
  _G.AutoZou = Value
end

function ModuleChestTW2(Value)
  _G.Auto_Tushita = Value
end

function ModuleChestTW3(Value)
  _G.Auto_Yama = Value
end

function ModuleChestTW4(Value)
  _G.Auto_Soul_Guitar = Value
end

function ModuleChestTW5(Value)
  _G.AutoMatSoul = Value
end

function ModuleChestTW6(Value)
  _G.CDK = Value
end

function ModuleChestTW7(Value)
  _G.CDK_YM = Value
end

function ModuleChestTW8(Value)
  _G.CDK_TS = Value
end

function ModuleChestTW9()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LegendarySwordDealer","1")
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LegendarySwordDealer","2")
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LegendarySwordDealer","3")
end

function ModuleChestBP0()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("MysteriousMan","2")
end

function ModuleChestBP1(Value)
  _G.Tp_LgS = Value
end

function ModuleChestBP2(Value)
  _G.AutoPole = Value
end

function ModuleChestBP3(Value)
  _G.AutoPoleV2 = Value
end

function ModuleChestBP4(Value)
  _G.AutoSaw = Value
end

function ModuleChestBP5(Value)
  _G.AutoSaber = Value
end

function ModuleChestBP6(Value)
  _G.AutoColShad = Value
end

function ModuleChestBP7(Value)
  _G.AutoGetUsoap = Value
end

function ModuleChestBP8(Value)
  _G.Greybeard = Value
end

function ModuleChestBP9(Value)
  _G.WardenBoss = Value
end

function ModuleWeaponDropdown00(Value)
  _G.MarinesCoat = Value
end

function ModuleWeaponDropdown01(Value)
  _G.SwanCoat = Value
end

function ModuleWeaponDropdown02(Value)
  _G.IceBossRen = Value
end

function ModuleWeaponDropdown03(Value)
  _G.KeysRen = Value
end

function ModuleWeaponDropdown04(Value)
  _G.AutoTridentW2 = Value
end

function ModuleWeaponDropdown05(Value)
  _G.LongsWord = Value
end

function ModuleWeaponDropdown06(Value)
  _G.BlackSpikey = Value
end

function ModuleWeaponDropdown07(Value)
  _G.DarkBladev3 = Value
end

function ModuleWeaponDropdown08(Value)
  _G.AutoEcBoss = Value
end

function ModuleWeaponDropdown09(Value)
  _G.Auto_Def_DarkCoat = Value
end

function ModuleWeaponDropdown10(Value)
  _G.Auto_DonAcces = Value
end

function ModuleWeaponDropdown11(Value)
  _G.Auto_SwanGG = Value
end

function ModuleWeaponDropdown12(Value)
  _G.AutoBigmom = Value
end

function ModuleWeaponDropdown13(Value)
  _G.Auto_Cavender = Value
end

function ModuleWeaponDropdown14(Value)
  _G.TwinHook = Value
end

function ModuleWeaponDropdown15(Value)
  _G.AutoSerpentBow = Value
end

function ModuleWeaponDropdown16(Value)
  _G.AutoKilo = Value
end

function ModuleWeaponDropdown17(Value)
  _G.Tp_MasterA = Value
end

function ModuleWeaponDropdown18()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ColorsDealer","2")
end

function ModuleWeaponDropdown19(Value)
  _G.Auto_Rainbow_Haki = Value
end

function ModuleWeaponDropdown20(Value)
  _G.GetQFast = Value
end

function ModuleWeaponDropdown21(Value)
  _G.obsFarm = Value
end

function ModuleWeaponDropdown22(Value)
  _G.AutoKenVTWO = Value
end

function ModuleWeaponDropdown23(Value)
  _G.Bartilo_Quest = Value
end

function ModuleWeaponDropdown24(Value)
  _G.CitizenQuest = Value
end

function ModuleWeaponDropdown25(Value)
  _G.DummyMan = Value
end

function ModuleWeaponDropdown26(Value)
  _G.Auto_SuperHuman = Value
end

function ModuleWeaponDropdown27(Value)
  _G.AutoDeathStep = Value
end

function ModuleWeaponDropdown28(Value)
  _G.Auto_SharkMan_Karate = Value
end

function ModuleWeaponDropdown29(Value)
  _G.Auto_Electric_Claw = Value
end

function ModuleWeaponDropdown30(Value)
  _G.AutoDragonTalon = Value
end

function ModuleWeaponDropdown31(Value)
  _G.Auto_God_Human = Value
end

function ModuleWeaponDropdown32(Value)
  _G.Snaguine = Value
end

function ModuleWeaponDropdown33(Value)
  _G.FindMirage = Value
end

function ModuleWeaponDropdown34(Value)
...
function ModuleWeaponDropdown35(Value)
        _G.AutoMysticIsland = Value
    end

function ModuleWeaponDropdown36(Value)
  _G.HighestMirage = Value
end

function ModuleWeaponDropdown37(Value)
  _G.TPGEAR = Value
end

function ModuleWeaponDropdown38(Value)
  _G.can = Value
end

function ModuleWeaponDropdown39(Value)
  _G.Addealer = Value
end

function ModuleWeaponDropdown40(Value)
  _G.FarmChestM = Value
end

function ModuleWeaponDropdown41()
...
function ModuleWeaponDropdown42(Value)
  LookM = Value
end

function ModuleWeaponDropdown43(Value)
        LookMV3 = Value
    end

function ModuleWeaponDropdown44(Value)
  _G.Auto_Mink = Value
end

function ModuleWeaponDropdown45(Value)
  _G.Auto_Human = Value
end

function ModuleWeaponDropdown46(Value)
  _G.Auto_Skypiea = Value
end

function ModuleWeaponDropdown47(Value)
  _G.Auto_Fish = Value
end

function ModuleWeaponDropdown48(Value)
  _G.Lver = Value
end

function ModuleWeaponDropdown49(Value)
  _G.AcientOne = Value
end

function ModuleWeaponDropdown50()
...
function ModuleWeaponDropdown51()
        local plr = game:GetService("Players").LocalPlayer
        local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
...
function ModuleWeaponDropdown52()
        local plr = game:GetService("Players").LocalPlayer
        local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
...
function ModuleWeaponDropdown53(Value)
  _G.TPDoor = Value
end

function ModuleWeaponDropdown54(Value)
  _G.Complete_Trials = Value
end

function ModuleWeaponDropdown55(Value)
  _G.Defeating = Value
end

function ModuleWeaponDropdown56()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5661.5322265625, 1013.0907592773438, - 334.9649963378906))
        topos(CFrame.new(5814.42724609375, 1208.3267822265625, 884.5785522460938))
    end

function ModuleWeaponDropdown57(Value)
  _G.Dojoo = Value
end

function ModuleWeaponDropdown58(Value)
  _G.FarmBlazeEM = Value
end

function ModuleWeaponDropdown59(Value)
  _G.UPGDrago = Value
end

function ModuleWeaponDropdown60(Value)
  _G.DragoV1 = Value
end

function ModuleWeaponDropdown61(Value)
  _G.AutoFireFlowers = Value
end

function ModuleWeaponDropdown62(Value)
  _G.DragoV3 = Value
end

function ModuleWeaponDropdown63(Value)
  _G.Relic123 = Value
end

function ModuleWeaponDropdown64(Value)
  _G.TrainDrago = Value
end

function ModuleWeaponDropdown65(Value)
  _G.TpDrago_Prehis = Value
end

function ModuleWeaponDropdown66(Value)
  _G.BuyDrago = Value
end

function ModuleWeaponDropdown67(Value)
  _G.DT_Uzoth = Value
end

function ModuleWeaponDropdown68()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "Dragonheart"
...
function ModuleWeaponDropdown69()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "Dragonstorm"
...
function ModuleWeaponDropdown70()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "DinoHood"
...
function ModuleWeaponDropdown71()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "TRexSkull"
...
function ModuleWeaponDropdown72()
...
function ModuleWeaponDropdown73(Value)
        getgenv().AutoCraftVolcanic = Value
    end

function ModuleWeaponDropdown74(Value)
        _G.Prehis_Find = Value
    end

function ModuleWeaponDropdown75(Value)
        _G.AutoStartPrehistoric = Value
    end

function ModuleWeaponDropdown76(Value)
        _G.Prehis_Skills = Value
    end

function ModuleWeaponDropdown77(Value)
    _G.KillAuraFull = Value
end

function ModuleWeaponDropdown78(Value)
  _G.Prehis_DB = Value
end

function ModuleWeaponDropdown79(Value)
  _G.Prehis_DE = Value
end

function ModuleWeaponDropdown80(Value)
  _G.ResetPH = Value
end

function ModuleWeaponDropdown81()
        game:GetService("Lighting").BaseAtmosphere:Destroy()
    end

function ModuleWeaponDropdown82(Value)
        getgenv().SpeedBoat = Value
    end

function ModuleWeaponDropdown83(Value)
        SetSpeedBoat = Value
    end

function ModuleWeaponDropdown84(Value)
        getgenv().AutoPressW = Value
    end

function ModuleWeaponDropdown85(Value)
        getgenv().NoClipShip = Value
    end

function ModuleWeaponDropdown86()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "SharkTooth"
...
function ModuleWeaponDropdown87()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "TerrorJaw"
...
function ModuleWeaponDropdown88()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "SharkAnchor"
...
function ModuleWeaponDropdown89()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LeviathanCrown"
...
function ModuleWeaponDropdown90()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LeviathanShield"
...
function ModuleWeaponDropdown91()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LeviathanBoat"
...
function ModuleWeaponDropdown92()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LegendaryScroll"
...
function ModuleWeaponDropdown93()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "MythicalScroll"
...
function ModuleWeaponDropdown94(Value)
        _G.SelectedBoat = Value
    end

function ModuleWeaponDropdown95()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyBoat",_G.SelectedBoat)
end

function ModuleWeaponDropdown96(Value)
  _G.DangerSc = Value
end

function ModuleWeaponDropdown97(Value)
  _G.SailBoats = Value
end

function ModuleWeaponDropdown98(Value)
  _G.Shark = Value
end

function ModuleWeaponDropdown99(Value)
  _G.Piranha = Value
end

function ModuleUI200(Value)
  _G.TerrorShark = Value
end

function ModuleUI201(Value)
  _G.MobCrew = Value
end

function ModuleUI202(Value)
  _G.HCM = Value
end

function ModuleUI203(Value)
  _G.PGB = Value
end

function ModuleUI204(Value)
  _G.FishBoat = Value
end

function ModuleUI205(Value)
  _G.SeaBeast1 = Value
end

function ModuleUI206(Value)
  _G.AutofindKitIs = Value
end

function ModuleUI207(Value)
  _G.tweenShrine = Value
end

function ModuleUI208(Value)
  _G.Collect_Ember = Value
end

function ModuleUI209(Value)
  _G.Trade_Ember = Value
end

function ModuleUI210()
  replicated.Modules.Net:FindFirstChild("RF/KitsuneStatuePray"):InvokeServer()
end

function ModuleUI211()
  replicated.Modules.Net:FindFirstChild("RE/TouchKitsuneStatue"):FireServer()
end

function ModuleUI212()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("InfoLeviathan", "2")
    end

function ModuleUI213(Value)
  _G.FrozenTP = Value
end

function ModuleUI214(Value)
  _G.SailBoat_Hydra = Value
end

function ModuleUI215(Value)
  _G.Leviathan1 = Value
end

function ModuleUI216(Value)
        BerryEsp = Value
        if not Value then
            for _, v in ipairs(workspace:GetChildren()) do
                if v:IsA("Part") and v.Name:match("BerryEspPart_.*") then
...
function ModuleUI227(Value)
  _G.Random_Auto = Value
end

function ModuleUI228(Value)
  _G.DropFruit = Value
end

function ModuleUI229(Value)
  _G.StoreF = Value
end

function ModuleUI230(Value)
  _G.TwFruits = Value
end

function ModuleUI231(Value)
  _G.InstanceF = Value
end

function ModuleUI232(Value)
        getgenv().SelectFruit = Value
    end

function ModuleUI233(Value)
        getgenv().AutoBuyFruitSniper = Value
    end

function ModuleUI234(Value)
  _G.SelectChip = Value
end

function ModuleUI235(Value)
  _G.AutoSelectDungeon = Value
end

function ModuleUI236(Value)
        getgenv().AutoGetFruit = Value
    end

function ModuleUI237()
  if not GetBP("Special Microchip") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaidsNpc","Select", "_G.SelectChip") end
end

function ModuleUI238()
  ...
  for _,y in pairs(FruitPrice) do    
    for i,v in pairs(DungeonTables) do 
      if not GetBP("Special Microchip") then     
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadFruit",tostring(y))	      
	    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaidsNpc","Select", "_G.SelectChip")	
	  end            
    end    
  end
end

function ModuleUI239(Value)
    _G.AutoChipBeli = Value
end

function ModuleUI240(Value)
    _G.AutoChipFruit = Value
end

function ModuleUI241(Value)
        _G.Auto_StartRaid = Value
    end

function ModuleUI242(Value)
        _G.Raiding = Value
    end

function ModuleUI243(Value)
  _G.Auto_Awakener = Value
end

function ModuleUI244(Value)
        _G.TpLab = Value
        StopTween(_G.TpLab)
        while _G.TpLab do
            wait()
...
function ModuleUI245()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BlackbeardReward","Microchip","2")
end

function ModuleUI246()
  fireclickdetector(workspace.Map.CircleIsland.RaidSummon.Button.Main.ClickDetector)
end

function ModuleUI247(Value)
        getgenv().AutoBuyMicrochipLaw = Value
    end

function ModuleUI248(Value)
        getgenv().AutoStartLawRaids = Value
    end

function ModuleUI249(Value)
  _G.AutoLawKak = Value
end

function ModuleUI250(Value)
        _G.AutoFarmDungeon = Value
    end

function ModuleUI251(v)
        _G.TPFloor1 = v
    end

function ModuleUI252(v)
        _G.TPFloor2 = v
    end

function ModuleUI253(v)
        _G.TPFloor3 = v
    end

function ModuleUI254(v)
        _G.TPFloor4 = v
    end

function ModuleUI255(Value)
        _G.PlayersList = Value
    end

function ModuleUI256(Value)
        _G.TpPly = Value
        spawn(function()
            pcall(function()
                while _G.TpPly do
...
function ModuleUI258(Value)
        ABmethod = Value
    end

function ModuleUI259(Value)
        _G.AimMethod = Value
    end

function ModuleUI260(Value)
        _G.AimCam = Value
    end

function ModuleUI261()
        pcall(function()
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("PlayerHunter")
        end)
    end

function ModuleUI262(Value)
        _G.AutoReceivePlayerQuest = Value
    end

function ModuleUI263(Value)
        _G.AutoPlayerHunter = Value
    end

function ModuleUI264(Value)
        _G.AutoPvP = Value
    end

function ModuleUI265(Value)
        _G.SafeMode = Value
    end

function ModuleUI266(Value)
        toggleFly(Value)
    end

function ModuleUI267(Value)
        updateFlySpeed(Value)
    end

function ModuleUI268(Value)
        getgenv().DodgeNoCD = Value
    end

function ModuleUI269(Value)
        InfAblities = Value
    end

function ModuleUI270(Value)
        infEnergy = Value
        if Value then
...
function ModuleUI273(Value)
        _G.NoAimTeam = Value
    end

function ModuleUI274(Value)
        _G.AcceptAlly = Value
    end

function ModuleUI275()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TravelMain")
end

function ModuleUI276()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TravelDressrosa")
end

function ModuleUI277()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TravelZou")
end

function ModuleUI278(Value)
  _G.Island = Value
end

function ModuleUI279(Value)
  _G.Teleport = Value
  if Value then
    for i,v in pairs(workspace["_WorldOrigin"].Locations:GetChildren()) do
...
function ModuleUI280(Value)
  _G.Island_PT = Value
end

function ModuleUI281()
  if _G.Island_PT == "Sky" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(-7894, 5547, -380))
  elseif _G.Island_PT == "UnderWater" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(61163, 11, 1819))
  elseif _G.Island_PT == "SwanRoom" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(2285, 15, 905))
  elseif _G.Island_PT == "Cursed Ship" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(923, 126, 32852))
  ...
  elseif _G.Island_PT == "Mansion Cafe" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(-12471.169921875, 374.94024658203, -7551.677734375))
  elseif _G.Island_PT == "Hydra Teleport" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(5643.45263671875, 1013.0858154296875, -340.51025390625))
  elseif _G.Island_PT == "Canvendish Room" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(5314.54638671875, 22.562219619750977, -127.06755065917969))
  elseif _G.Island_PT == "Temple of Time" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(28310.0234, 14895.1123, 109.456741, -0.469690144, -2.85620132e-08, -0.882831335, -3.23509219e-08, 1, -1.51411736e-08, 0.882831335, 2.14487486e-08, -0.469690144))
  end
end

function ModuleUI282(Value)
  NPClist = Value
end

function ModuleUI283(Value)
  _G.TPNpc = Value
end

function ModuleUI284(Value)
        SelectedMelee = Value
    end

function ModuleUI285(Value)
        ...
        -- Bật noclip khi character được reset
        ...
        task.spawn(function()
            ...
            while IsRunning do
                ...
                -- Tăng timeout nếu không đến được vị trí
                timeout = timeout + 1
                if timeout >= 60 then -- 1 phút
                    TurnOffToggle()
                    break
                end
            end
...
function ModuleUI286()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Tomoe Ring")
end

function ModuleUI287()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Black Cape")
end

function ModuleUI288()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Swordsman Hat")
end

function ModuleUI289()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Ectoplasm","Buy", 1)
end

function ModuleUI290()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Ectoplasm","Buy", 2)
end

function ModuleUI291()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Cutlass")
end

function ModuleUI292()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Katana")
end

function ModuleUI293()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Iron Mace")
end

function ModuleUI294()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Duel Katana")
end

function ModuleUI295()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Triple Katana")
end

function ModuleUI296()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Pipe")
end

function ModuleUI297()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Dual-Headed Blade")
end

function ModuleUI298()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Bisento")
end

function ModuleUI299()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Soul Cane")
end

function ModuleFarmLevel00()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Slingshot")
end

function ModuleFarmLevel01()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Musket")
end

function ModuleFarmLevel02()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Dual Flintlock")
end

function ModuleFarmLevel03()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Flintlock")
end

function ModuleFarmLevel04()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Refined Flintlock")
end

function ModuleFarmLevel05()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyItem","Cannon")
end

function ModuleFarmLevel06()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BlackbeardReward","Slingshot","2")
end

function ModuleFarmLevel07()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BlackbeardReward","Refund","2")
end

function ModuleFarmLevel08()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BlackbeardReward","Reroll","2")
end

function ModuleFarmLevel09()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Ectoplasm"," Change", 4)
end

function ModuleFarmLevel10()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CyborgTrainer"," Buy")
end

function ModuleFarmLevel11()
...
function ModuleFarmLevel13()
  game:GetService("TeleportService"):Teleport(game.PlaceId, game.Players.LocalPlayer)
end

function ModuleFarmLevel14()
        loadstring(game:HttpGet("https://pastefy.app/b6U8AEbv/raw"))()
    end

function ModuleFarmLevel15()
  loadstring(game:HttpGet("https://pastefy.app/b6U8AEbv/raw"))()
end

function ModuleFarmLevel16()
    loadstring(game:HttpGet("https://pastefy.app/b6U8AEbv/raw"))()
end

function ModuleFarmLevel17(Value)
        getgenv().Job = Value
    end

function ModuleFarmLevel18()
...
function ModuleFarmLevel19()
  setclipboard(tostring(game.JobId))
end

function ModuleFarmLevel20()
  plr.PlayerGui.Main.AwakeningToggler.Visible = true
end

function ModuleFarmLevel21()
  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("getTitles",true)
  plr.PlayerGui.Main.Titles.Visible = true
end

function ModuleFarmLevel22(Value)
  _G.Rechat = Value
...
function ModuleFarmLevel23(Value)
  ReLeader = Value
...
function ModuleFarmLevel24()
  Pirates()
end

function ModuleFarmLevel25()
  Marines()
end

function ModuleFarmLevel26(Value)
  _G.PortalUnLock = Value
end

function ModuleFarmLevel27(Value)
  _G.SelectStateHaki = Value
end

function ModuleFarmLevel28()
  if _G.SelectStateHaki == "State 0" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ChangeBusoStage",0)
  elseif _G.SelectStateHaki == "State 1" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ChangeBusoStage",1)
  elseif _G.SelectStateHaki == "State 2" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ChangeBusoStage",2)
  elseif _G.SelectStateHaki == "State 3" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ChangeBusoStage",3)
  elseif _G.SelectStateHaki == "State 4" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ChangeBusoStage",4)
  elseif _G.SelectStateHaki == "State 5" then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ChangeBusoStage",5)
  end
end

function ModuleFarmLevel29(Value)
  _G.RTXMode = Value
...
function ModuleFarmLevel35(Value)
  bright = Value
  if Value == true then
...
function ModuleFarmLevel36(Value)
  _G.SelectDN = Value
end

function ModuleFarmLevel37(Value)
  _G.daylightN = Value
end

function ModuleFarmLevel38(Value)
...
function ModuleFarmLevel39(Value)
  _G.WalkWater = Value
end
-- Public API shared with NejiHubUI.lua.
local NejiHubCore = {
    Version = "6-split",
    World1 = World1,
    World2 = World2,
    ...
    Remotes = Remotes,
    GetQuests = GetQuests,
    GetQuestPoint = GetQuestPoint,
    QuestNeta = QuestNeta,
    MaterialMon = MaterialMon,
    EquipWeapon = EquipWeapon,
    weaponSc = weaponSc,
    statsSetings = statsSetings,
    BringEnemy = BringEnemy,
    IsInSubmergedIsland = IsInSubmergedIsland,
}


-- Runtime loops moved from the UI chunk.
spawn(function()
    while true do
        ModuleTimeZone()
        wait(1)
  ...
  while wait() do
    pcall(function()
      if _G.AutoFarmNear then
        ...
        -- Tìm mob gần nhất trong phạm vi 30 studs
        for i,v in pairs(workspace.Enemies:GetChildren()) do
        ...
        -- Nếu có mob trong tầm 30 studs thì tấn công nó
        if closestMob then
          repeat 
            wait() 
            Attack.Kill(closestMob, _G.AutoFarmNear)
          until not _G.AutoFarmNear or not closestMob.Parent or closestMob.Humanoid.Health <= 0
        else
          -- Nếu không có mob trong tầm 30 studs, tìm mob gần nhất để di chuyển tới
          local targetMob = nil
          ...
          -- Di chuyển đến mob gần nhất bằng _tp
          if targetMob then
            local character = game.Players.LocalPlayer.Character
            local targetPosition = targetMob.HumanoidRootPart.Position + Vector3.new(0, 0, 5)
            
            -- Dùng _tp để di chuyển
            _tp(CFrame.new(targetPosition))
            
            wait(0.5) -- Chờ di chuyển xong
          end
        end
      end
    end)
...
spawn(function()
    while wait(Sec) do
        pcall(function()    
            Process:SetDesc("Elite Progress : " .. game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("EliteHunter", "Progress"))
        end)
    end
end)
        ...
        pcall(function()
            if _G.FarmEliteHunt then
                ...
                if not questGui.Visible then
                    ...
                    local result = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("EliteHunter")
...
spawn(function()
    while wait(0.2) do
        pcall(function()
            local Killed = string.match(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CakePrinceSpawner"), "%d+")
            if Killed then
...
spawn(function()
    ...
    local hasBrought = false

    while task.wait(0.3) do
        pcall(function()
            if not _G.Auto_Cake_Prince or _G.AutoRaidCastle then return end
            ...
            if _G.AcceptQuestC and questUI and not questUI.Visible then
                ...
                _tp(questPos)
                while _G.Auto_Cake_Prince and (questPos.Position - root.Position).Magnitude > 50 do
                    task.wait(0.2)
                end
                ...
                if enemies then
                    for _, v in ipairs(enemies:GetChildren()) do
                        if v:IsA("Model") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") then
                            local isCakeMob = false
                            for _, name in ipairs(CakeMobs) do
                                if v.Name == name then isCakeMob = true break end
                            end
                            if isCakeMob then
            ...
            -- 3. XỬ LÝ MỤC TIÊU
            if target then
                -- TWEEN đến mục tiêu (bay cao 20)
                local flyHeight = 20
                local targetPos = target.HumanoidRootPart.CFrame * CFrame.new(0, flyHeight, 0)

                -- Gọi tween và chờ hoàn tất
                tweenTo(targetPos, 1.5)

                -- SAU KHI TWEEN XONG, MỚI BRING (chỉ 1 lần)
                if not isBoss and not hasBrought then
                    local bringPosition = root.Position  -- vị trí sau khi tween
                    ...
                    if enemies then
                        ...
                        for _, v in ipairs(enemies:GetChildren()) do
                            if v.Name == mobName and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") then
                                v.HumanoidRootPart.CFrame = CFrame.new(bringPosition)
                                count = count + 1
                                task.wait()
                            end
                        end
                ...
                -- ĐÁNH MỤC TIÊU ĐẾN KHI CHẾT
                while _G.Auto_Cake_Prince and target and target.Parent and target.Humanoid.Health > 0 do
                    if not isBoss and (bigMirror and bigMirror.Other.Transparency == 0 or (enemies and enemies:FindFirstChild("Cake Prince"))) then
                        break
                    end
                    if isBoss and (not target.Parent or target.Humanoid.Health <= 0) then
                        break
                    end

                    if isBoss then
                        Attack.Kill2(target, _G.Auto_Cake_Prince)
                    else
                        Attack.Kill(target, _G.Auto_Cake_Prince)
                    end
                    task.wait()
                end

                -- SAU KHI TARGET CHẾT
                if not target.Parent or target.Humanoid.Health <= 0 then
                    if not isBoss then
                        -- Tìm mob trong bán kính 40
                        local nearbyMob = nil
                        ...
                        if nearbyMob then
                            -- Có mob gần -> giữ hasBrought = true (không bring lại)
                            lastTarget = nearbyMob
                            hasBrought = true
                        else
                            -- Không có mob gần -> reset để lần tới bring
                            lastTarget = nil
                            hasBrought = false
                        end
                    else
                ...
                -- KHÔNG CÓ MỤC TIÊU -> TWEEN VỀ ĐIỂM CHỜ
                local waitPos = CFrame.new(-2077, 252, -12373)
                tweenTo(waitPos, 1.5)

                lastTarget = nil
                hasBrought = false

                while _G.Auto_Cake_Prince and not enemies:FindFirstChildOfClass("Model") do
                    task.wait(0.5)
                end
            end
        end)
            ...
            pcall(function()
                if not workspace.Map.CakeLoaf:FindFirstChild("RedDoor") then
                    if GetBP("Red Key") then
                        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CakeScientist", "Check")
                        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaidsNpc", "Check")
                    end
                ...
                if GetBP("God's Chalice") and GetM("Conjured Cocoa") >= 10 then
                    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("SweetChaliceNpc")
                end
...
spawn(function()        
    -- Biến để kiểm soát thời gian bring
    local lastBringTime = 0
    local bringInterval = 0.5 -- Bring mỗi 0.5 giây
    ...
    while task.wait(0.3) do
        if _G.AutoFarm_Bone then 
            pcall(function()
                ...
                -- 3. XỬ LÝ BRING MOB THEO LOẠI (MỖI 0.5 GIÂY)
                if targetMob then
                    local currentTime = tick()
                    
                    -- Chỉ bring nếu đã qua 0.5 giây
                    if currentTime - lastBringTime >= bringInterval then
                        local mobName = targetMob.Name
                        local bringPosition = nil
                        
                        -- Xác định vị trí bring theo từng loại quái
                        if mobName == "Reborn Skeleton" then
                            bringPosition = Vector3.new(-8755.28, 142.42, 5977.86)
                        elseif mobName == "Demonic Soul" then
                            bringPosition = Vector3.new(-9512.19, 172.42, 6180.58)
                        elseif mobName == "Living Zombie" then
                            bringPosition = Vector3.new(-10110.87, 138.94, 5977.52)
                        elseif mobName == "Possessed Mummy" then
                            -- Tăng Y lên 85 để nhân vật có thể tween đến
                            bringPosition = Vector3.new(-9564.89, 85.10, 6220.95)
                        end
                        
                        -- Thực hiện bring nếu có vị trí
                        if bringPosition and enemies then
                            local children = enemies:GetChildren()
                            local count = 0
                            for i = 1, #children do
                                local v = children[i]
                                if v.Name == mobName and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") then
                                    -- Teleport từng con về vị trí tập trung
                                    v.HumanoidRootPart.CFrame = CFrame.new(bringPosition)
                                    count = count + 1
                                    task.wait()
                                end
                            ...
                            -- Cập nhật thời gian bring nếu có bring
                            if count > 0 then
                ...
                -- 4. Di chuyển và đánh quái
                if targetMob then
                    -- Kiểm tra nếu target là Possessed Mummy thì bay đến vị trí cao hơn
                    local flyHeight = 20
                    local targetCFrame = targetMob.HumanoidRootPart.CFrame
                    
                    if targetMob.Name == "Possessed Mummy" then
                        -- Bay đến vị trí trên cao để có thể tween xuống
                        flyHeight = 50
                        -- Di chuyển đến vị trí bring + bay cao
                        _tp(CFrame.new(-9564.89, 85.10, 6220.95) * CFrame.new(0, flyHeight, 0))
                    else
                        -- Bay đến quái bình thường
                        _tp(targetCFrame * CFrame.new(0, flyHeight, 0))
                    end
      ...
      pcall(function()
	    ...
	    if v then
          ...
          if not GetBP("Hallow Essence") then
            repeat task.wait(.1)game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Bones","Buy",1,1)until _G.AutoHytHallow == false or GetBP("Hallow Essence")
          else
  ...
  while wait(Sec) do
    pcall(function()
      if _G.Auto_Random_Bone then    
  	    repeat task.wait() game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Bones","Buy",1,1) until not _G.Auto_Random_Bone
      end
    end)
  end
end)
  ...
  while wait(Sec) do
    pcall(function()
      if _G.FarmMastery_S then
        if SelectIsland == "Cake" then
          for _, v in next, game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("getInventory") do          
              ...
              if v.Type == "Sword" then
                ...
                if tonumber(v.Mastery) >= 1 or tonumber(v.Mastery) <= 599 then
                  ...
                  if GetBP(SwordName) then                    
                    ...
                    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadItem",SwordName)   
                  end   
              elseif tonumber(v.Mastery) >= 600 then
                if GetBP(SwordName) then return nil else game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadItem",SwordName) end       
              end
                break
              end
            end         
  ...
  while wait(Sec) do
    pcall(function()
      ...
      local _HasBuso = {"HasBuso","Buso"}
  	  if not plr.Character:FindFirstChild(_HasBuso[1]) then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer(_HasBuso[2]) end
      end
    end)
  end
end)
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoZou then
          ...
          if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BartiloQuestProgress","Bartilo") == 3 then
            if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GetUnlockables").FlamingoAccess ~= nil then
              game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","TravelZou")
              if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ZQuestProgress", "Check") == 0 then
                local v = GetConnectionEnemies("rip_indra")
                if v then
                  repeat wait() Attack.Kill(v,_G.AutoZou) until not _G.AutoZou or not v.Parent or v.Humanoid.Health <= 0
                  Check = 2
                  repeat wait()game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","TravelZou")until Check == 1                   
                else
                  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","ZQuestProgress","Check") wait(.1)
                  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","ZQuestProgress","Begin")
                end
            ...
            if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GetUnlockables").FlamingoAccess == nil then
              ...
              for i,DevilFruitOpenDoor in pairs(TabelDevilFruitOpen) do
                for i1,DevilFruitStore in pairs(TabelDevilFruitStore) do
                  if DevilFruitOpenDoor == DevilFruitStore and game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GetUnlockables").FlamingoAccess == nil then
                    if not plr.Backpack:FindFirstChild(DevilFruitStore) then
                      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","LoadFruit",DevilFruitStore)
                    else
                      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","TalkTrevor","1")
                      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","TalkTrevor","2")
                      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("F_","TalkTrevor","3")
                    end
                  end
                end
              end
            ...
            if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BartiloQuestProgress","Bartilo") == 0 then
            ...
            elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BartiloQuestProgress","Bartilo") == 1 then
            ...
            elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BartiloQuestProgress","Bartilo") == 2 then
  ...
  while wait(Sec) do
    pcall(function()
      if _G.Auto_Yama then
	    if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("EliteHunter", "Progress") < 30 then
	      _G.FarmEliteHunt = true
	    elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("EliteHunter", "Progress") > 30 then
	      _G.FarmEliteHunt = false
  ...
  while wait(Sec) do
    pcall(function()
      if _G.Auto_Soul_Guitar then
        if World3 then
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("gravestoneEvent", 2)
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("gravestoneEvent", 2, true)
          if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress","Check") == nil then
            _tp(CFrame.new(-8655.0166015625, 141.3166961669922, 6160.0224609375))
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("gravestoneEvent", 2)
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("gravestoneEvent", 2, true)
           elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress","Check").Swamp == false then
             Quest1 = true;
             Quest2 = false;
             Quest3 = false;
             Quest4 = false;
           ...
           elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress","Check").Gravestones == false then
             Quest1 = false;
             Quest2 = true;
             Quest3 = false;
             Quest4 = false;
           ...
           elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress","Check").Ghost == false then
             game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress", "Ghost")
             game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress", "Ghost", true)
           elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress","Check").Trophies == false then
             Quest1 = false;
             Quest2 = false;
             Quest3 = true;
             Quest4 = false;             
        ...
        until workspace.Map["Haunted Castle"].Tablet.Segment2.Line.Rotation.Z == 0 or workspace.Map["Haunted Castle"].Tablet.Segment5.Line.Rotation.Z == 0 or workspace.Map["Haunted Castle"].Tablet.Segment6.Line.Rotation.Z == 0 or workspace.Map["Haunted Castle"].Tablet.Segment8.Line.Rotation.Z == 0 or workspace.Map["Haunted Castle"].Tablet.Segment9.Line.Rotation.Z == 0
          elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GuitarPuzzleProgress","Check").Pipes == false then
            Quest1 = false;
            Quest2 = false;
            Quest3 = false;
            Quest4 = true;
	    ...
	    if GetM("Bones") >= 500 and GetM("Ectoplasm") >= 250 and GetM("Dark Fragment") >= 1 then
		  ...
		  if GetM("Ectoplasm") <= 250 then
		    if _G.AutoMatSoul and World2 then
			  ...
			  else game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(923.21252441406, 126.9760055542, 32852.83203125))
			  end
		    else game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TravelDressrosa")
		    end
  ...
  while wait(Sec) do
    pcall(function()
      if _G.CDK then
        ...
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","StartTrial","Boss")
...
spawn(function()
  while wait() do
    pcall(function()
      if _G.CDK_YM then
        if tostring(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest", "OpenDoor")) ~= "opened" then                  
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest", "OpenDoor")
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest", "OpenDoor", true)
        else
          if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Finished"] == nil then
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","StartTrial","Evil")
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","StartTrial","Evil")
          elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Finished"] == false then                        
            if tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == -3 then
              ...
              until tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 1 or not _G.CDK_YM
            elseif tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == -4 then
                    ...
                    if (CFramePos.Position - Root.Position).Magnitude <= 1000 and workspace.Enemies:FindFirstChild(NameMonHaze) then
                      for i,v in pairs(workspace.Enemies:GetChildren()) do
                        if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 and v:FindFirstChild("HazeESP") then
                          repeat wait() Attack.Kill(v, _G.CDK_YM) until not _G.CDK_YM or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 2 or not v:FindFirstChild("HazeESP") or v.Humanoid.Health <= 0
                        end
                      end
                    else   
                      _tp(CFramePos)                               
            ...
            elseif tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == -5 then
                ...
                if (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 1000 then
                  for gg,ez in pairs(workspace.Map.HellDimension.Exit:GetChildren()) do
                    if tonumber(gg) == 2 then
                      repeat task.wait() Root.CFrame = workspace.Map.HellDimension.Exit.CFrame until not _G.CDK_YM or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                    end
                  end
                  EquipWeapon(_G.SelectWeapon)
                  if tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) ~= 3 then
                  ...
                  until not _G.CDK_YM or _G.T1Yama or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                  repeat task.wait()
                    repeat task.wait()
                  ...
                  until _G.T2Yama or _G.CDK_YM == false or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                    repeat wait()
                      repeat task.wait() 
                    ...
                    until _G.T3Yama or _G.CDK_YM == false or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                  end
                  for i,v in pairs(workspace.Enemies:GetChildren()) do
                    if (v:FindFirstChild("HumanoidRootPart").Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 300 then
                      if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 then
                        repeat task.wait() Attack.Kill(v,_G.CDK_YM) until not _G.CDK_YM or v.Humanoid.Health <= 0 or not v.Parent or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                      end
                    end
                  end
                end
...
spawn(function()
  while wait() do
    pcall(function()
      if _G.CDK_YM then
        if tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == -5 then
          if not workspace.Map:FindFirstChild("HellDimension") or (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude > 1000 then
            local v = GetConnectionEnemies("Soul Reaper")
            if v then repeat task.wait()_tp(v.HumanoidRootPart.CFrame) until v.Humanoid.Health <= 0 or not _G.CDK_YM or not v.Parent or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Evil"]) == 3 or (workspace.Map:FindFirstChild("HellDimension") and (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 1000)
            ...
            elseif replicated:FindFirstChild("Soul Reaper") and replicated:FindFirstChild("Soul Reaper").Humanoid.Health > 0 then
              ...
              if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Bones","Check") < 50 and not workspace.Enemies:FindFirstChild("Soul Reaper") and not replicated:FindFirstChild("Soul Reaper") and not workspace.Map:FindFirstChild("HellDimension") then
                ...
                game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Bones", "Buy", 1, 1)
              end
            end
          end
        end
...
spawn(function()
  while wait() do
    pcall(function()
      if _G.CDK_TS then
        if tostring(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest", "OpenDoor")) ~= "opened" then
          wait(.7) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest", "OpenDoor")
          wait(.3) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest", "OpenDoor", true)
        else
          if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Finished"] == nil then
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","StartTrial","Good")
          elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Finished"] == false then
            if tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == -3 then
              QuestTushita_1 = true
              QuestTushita_2 = false
              QuestTushita_3 = false
              repeat wait() _tp(CFrame.new(-4602.5107421875, 16.446542739868164, -2880.998046875)) until (CFrame.new(-4602.5107421875, 16.446542739868164, -2880.998046875).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.CDK_TS or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == 1
              if (CFrame.new(-4602.5107421875, 16.446542739868164, -2880.998046875).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 10 then
                wait(.7) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","BoatQuest",workspace.NPCs:FindFirstChild("Luxury Boat Dealer"),"Check")
                wait(.5) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","BoatQuest",workspace.NPCs:FindFirstChild("Luxury Boat Dealer"))
              end
                ...
                wait(.5) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","BoatQuest",workspace.NPCs:FindFirstChild("Luxury Boat Dealer"))
                  ...
                  wait(1)
                  elseif tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == -4 then
                    QuestTushita_1 = false
                    QuestTushita_2 = true
                    QuestTushita_3 = false
                    repeat wait()
                      _G.AutoRaidCastle = true
                    until not _G.CDK_TS or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == 2 
                      _G.AutoRaidCastle = false         
                  elseif tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == -5 then
                    QuestTushita_1 = false
                    QuestTushita_2 = false
                    QuestTushita_3 = true
                      ...
                      for i,v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == "Cake Queen" then
                          if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                            ...
                            until not _G.CDK_TS or not v.Parent or v.Humanoid.Health <= 0 or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == 3
                          end
                        end
                      end
                   ...
                   if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - workspace.Map.HeavenlyDimension.Spawn.Position).Magnitude <= 1000 then
                     ...
                     if Ex == 2 then
                       ...
                       until not _G.CDK_TS or tonumber(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CDKQuest","Progress")["Good"]) == 3
                    end
                   repeat wait()
                     repeat wait() 
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoPoleV2 then        
	   if not GetBP("Pole (1st Form)") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadItem","Pole (1st Form)") end
	   if not GetBP("Pole (2nd Form)") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadItem","Pole (2nd Form)") end      
	   ...
	   if GetBP("Rumble Fruit").AwakenedMoves:FindFirstChild("Z") and GetBP("Rumble Fruit").AwakenedMoves:FindFirstChild("X") and GetBP("Rumble Fruit").AwakenedMoves:FindFirstChild("C") and GetBP("Rumble Fruit").AwakenedMoves:FindFirstChild("V") and GetBP("Rumble Fruit").AwakenedMoves:FindFirstChild("F") then
		...
		if plr.Data.Fragments.Value >= 5000 then
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Thunder God", "Talk") wait(Sec)
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Thunder God", "Sure")
        end
        elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Awakener","Check") == nil or game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Awakener","Check") == 0 then
          _G.SelectChip = "Rumble"
          local Buying = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaidsNpc","Select",_G.SelectChip)
	      ...
	      if workspace.Map.Jungle.QuestPlates.Door.Transparency == 0 then
		    ...
		    if workspace.Map.Desert.Burn.Part.Transparency == 0 then
		      if plr.Backpack:FindFirstChild("Torch") or plr.Character:FindFirstChild("Torch") then
		        ...
		        if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","SickMan") ~= 0 then
		          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","GetCup")
			      wait(0.5)
			      EquipWeapon("Cup")
			      wait(0.5)
			      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","FillCup",plr.Character.Cup)
			      wait(Sec)
			      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","SickMan") 
		        else
		 	      if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","RichSon") == nil then
			        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","RichSon")
		          elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","RichSon") == 0 then
			     ...
			     elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","RichSon") == 1 then
			       game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","RichSon")
				   EquipWeapon("Relic")
	     ...
	     if workspace.Enemies:FindFirstChild("Saber Expert") or replicated:FindFirstChild("Saber Expert") then
	       for _,v in pairs(workspace.Enemies:GetChildren()) do
		     if v.Name == "Saber Expert" and Attack.Alive(v) then
			   repeat task.wait() Attack.Kill(v, _G.AutoSaber) until v.Humanoid.Health <= 0 or _G.AutoSaber == false
		       if v.Humanoid.Health <= 0 then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("ProQuestProgress","PlaceRelic") end		      
		      end
		    end
		  else
    ...
    if _G.Greybeard then
      pcall(function()
        ...
        elseif GetWP("Bisento") then
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadItem","Bisento")
...
spawn(function()
  while wait(Sec) do
    pcall(function()
      ...
      if not GetBP("Dark Blade") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadItem","Dark Blade") end
      ...
      if _G.AutoEcBoss then
	    ...
	    elseif GetM("Ectoplasm") <= 99 then
	      ...
	      if v then repeat wait()Attack.Kill(v, _G.AutoEcBoss) until not _G.AutoEcBoss or not v.Parent or v.Humanoid.Health <= 0
	        ...
	        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(923.21252441406, 126.9760055542, 32852.83203125)) wait(.5)
...
spawn(function()
  while wait(.1) do
    if _G.Auto_DonAcces then
      pcall(function()
        if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GetUnlockables").FlamingoAccess == nil and plr.Data.Level.Value >= 1500 then
          FruitPrice = {}
	      FruitStore = {}
		  for i,v in next,replicated:WaitForChild("Remotes").CommF_:InvokeServer("GetFruits") do
		    ...
		    for _,x in pairs(v) do
	          ...
	          for _,y in pairs(FruitPrice) do
		        for _,z in pairs(FruitStore) do
		          if y == z and game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GetUnlockables").FlamingoAccess == nil then
		            _G.StoreF = false
			      if not plr.Backpack:FindFirstChild(FruitStore) then
			        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadFruit",tostring(y))
			      else
			        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TalkTrevor","1")
			        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TalkTrevor","2")
			        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("TalkTrevor","3")
			      end
			    end
		      end 
		    end
    ...
    pcall(function()
      if _G.TwinHook then
	    ...
	    if v then repeat wait()Attack.Kill(v,_G.TwinHook)until not _G.TwinHook or v.Humanoid.Health <= 0
          ...
          game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(-12471.169921875, 374.94024658203, -7551.677734375)) wait(.2)
  ...
  pcall(function()
    while wait(Sec) do
      if _G.Auto_Rainbow_Haki then
        if plr.PlayerGui.Main.Quest.Visible == false then
          if _G.GetQFast then
            if plr.PlayerGui.Main.Quest.Visible == false then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("HornedMan","Bet") end     
          else
            ...
            elseif (plr.Character.HumanoidRootPart.CFrame == Rainbow1) then
              wait(1)
              game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("HornedMan","Bet")
            end
          end
  ...
  while wait(Sec) do
    if _G.AutoKenVTWO then
	  ...
	  elseif plr.PlayerGui.Main.Quest.Visible == false then
	    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CitizenQuestProgress","Citizen") wait(.1)
	    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("StartQuest","CitizenQuest",1)
	  end
	  if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CitizenQuestProgress","Citizen") == 2 then
	  ...
	  if not GetBP("Fruit Bowl") then   	    
	    ...
	    if plr.Backpack:FindFirstChild("Banana") and plr.Backpack:FindFirstChild("Apple") and plr.Backpack:FindFirstChild("Pineapple") or plr:FindFirstChild("Banana") and plr:FindFirstChild("Apple") and plr:FindFirstChild("Pineapple") then
		  ...
		  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CitizenQuestProgress","Citizen")	    			 
	    end
	      if plr.Backpack:FindFirstChild("Fruit Bowl") or plr.Character:FindFirstChild("Fruit Bowl") then
		    ...
		    elseif plr.Character.HumanoidRootPart.CFrame == Kv2Pos3 then
		      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("KenTalk2","Start") wait(.1)
		      game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("KenTalk2","Buy")
	        end			 		    
	      end
	    end
      end)
        ...
        if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BartiloQuestProgress","Bartilo") == 0 then
          ...
          if Qbart.Visible == true then
            ...
            if v then
              ...
              if x then
                repeat task.wait()
                  if not string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Swan Pirate")then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("AbandonQuest")
  ...
  while wait(Sec) do
    pcall(function()
      if _G.CitizenQuest then
        if Lv >= 1800 and game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CitizenQuestProgress").KilledBandits == false then
          if string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Forest Pirate") and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "50") and plr.PlayerGui.Main.Quest.Visible == true then
            ...
            if (Vector3.new(-12443.8671875, 332.40396118164, -7675.4892578125) - plr.Character.HumanoidRootPart.Position).Magnitude <= 30 then
              wait(1.5) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("StartQuest","CitizenQuest",1)
            end
          end
        elseif Lv >= 1800 and game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CitizenQuestProgress").KilledBoss == false then
          local v = GetConnectionEnemies("Captain Elephant")
        ...
        elseif Lv >= 1800 and game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("CitizenQuestProgress","Citizen") == 2 then
	  ...
	  local M_Frag = plr.Data.Fragments.Value
        if plr:FindFirstChild("WeaponAssetCache") then
          if not GetBP("Superhuman") then                    
            ...
            if (M_Beli >= 150000) then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyBlackLeg") end
            ...
            if (M_Beli >= 500000) then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyElectro") end
            ...
            if (M_Beli >= 750000) then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyFishmanKarate") end
            ...
            if (M_Frag >= 1500) then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BlackbeardReward","DragonClaw","2") end
      ...
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then  
          if not GetBP("Death Step") then          
            if not GetBP("Black Leg") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyBlackLeg") end
            if GetBP("Black Leg") and GetBP("Black Leg").Level.Value >= 400 then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyDeathStep") _G.Level = false elseif GetBP("Black Leg") and GetBP("Black Leg").Level.Value < 399 then _G.Level = true end
            ...
            if workspace.Map.IceCastle.Hall.LibraryDoor.PhoeyuDoor.Transparency == 0 then            
              if GetBP("Library Key") then repeat wait() _tp(CFrame.new(6371.2001953125, 296.63433837890625, -6841.18115234375)) until not _G.AutoDeathStep or (Root.Position == CFrame.new(6371.2001953125, 296.63433837890625, -6841.18115234375).Position)
		        if (Root.CFrame == CFrame.new(6371.2001953125, 296.63433837890625, -6841.18115234375)) then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyDeathStep") end     
      ...
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then  
          if not GetBP("Sharkman Karate") then          
            if not GetBP("Fishman Karate") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyFishmanKarate") end
            if GetBP("Fishman Karate") and GetBP("Fishman Karate").Level.Value >= 400 then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuySharkmanKarate") _G.Level = false elseif GetBP("Fishman Karate") and GetBP("Fishman Karate").Level.Value < 399 then _G.Level = true end
            if GetBP("Fishman Karate") or GetBP("Fishman Karate").Level.Value >= 400 then           
              if GetBP("Water Key") then
		        if string.find(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuySharkmanKarate"), "keys") then  
			      if GetBP("Water Key") then
...
spawn(function()
  while wait(Sec) do
    if _G.Auto_Electric_Claw then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then 
        if not GetBP("Electro") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyElectro") end        
          if GetBP("Electro") and GetBP("Electro").Level.Value >= 400 then
            if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyElectricClaw", "Start") == nil then notween(CFrame.new(-12548, 337, -7481)) end
...
spawn(function()
  while wait(Sec) do
    if _G.AutoDragonTalon then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then 
        if not GetBP("Dragon Claw") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BlackbeardReward","DragonClaw","2") end        
          if GetBP("Dragon Claw") and GetBP("Dragon Claw").Level.Value >= 400 then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Bones","Buy",1,1) game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyDragonTalon")
  ...
  while wait() do
    pcall(function()
      if _G.Auto_God_Human then
        if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyGodhuman",true) == "Bring me 20 Fish Tails, 20 Magma Ore, 10 Dragon Scales and 10 Mystic Droplets." then
        ...
        elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyGodhuman",true) == 3 then
          return nil
        else
...
spawn(function()
  while wait(Sec) do
    if _G.Snaguine then
      pcall(function()
        if not GetBP("Sanguine Art") then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Sanguine Art") end
        ...
        else game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuySanguineArt")
        end
      end)
    end
  end
      ...
      pcall(function()
        if not workspace["_WorldOrigin"].Locations:FindFirstChild("Mirage Island", true) then                
          ...
          if not myBoat then
            ...
            if (buyBoatCFrame.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("BuyBoat", _G.SelectedBoat) end
          else
  ...
  while wait(Sec) do
    pcall(function()
      if _G.Auto_Mink then
        if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Alchemist","1") ~= 2 then
          if game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Alchemist","1") == 0 then
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Alchemist","2")
          elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Alchemist","1") == 1 then
          ...
          elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Alchemist","1") == 2 then
	        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Alchemist","3")
	      end
        elseif game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Wenlocktoad","1") == 0 then
    ...
    pcall(function()
      if _G.TrainDrago then
	    ...
	    for i=1,#DragoM do
          if plr.Character:FindFirstChild("RaceEnergy").Value == 1 then
            vim1:SendKeyEvent(true, "Y", false, game)
            game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("UpgradeRace","Buy",2)
...
spawn(function()
    while wait(0.2) do
        pcall(function()
            local spycheck = string.match(game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("InfoLeviathan", "1"), "%d+")
            if spycheck then 
...
spawn(function()
  while wait(Sec) do
   	pcall(function()
      if _G.Random_Auto then local args = {"Cousin", "DLCBoxData"} game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer(unpack(args)) end 
    end)
  end
end)
...
task.spawn(function()
    while task.wait(1) do
        if _G.AutoChipFruit then
            pcall(function()
                if not GetBP("Special Microchip") then
                    local fruits = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("GetFruits")
                    local cheapest = nil
                    ...
                    if cheapest then
                        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("LoadFruit", tostring(cheapest))
                        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("RaidsNpc", "Select", "_G.SelectChip")
                    end
                end
            end)
...
spawn(function()
    while task.wait(0.3) do
        pcall(function()
            ...
            local isAuraIsland = false
            for _, auraIsland in ipairs(auraIslands) do
                if currentIsland == auraIsland then
                    isAuraIsland = true
                    break
                end
            end
            
            if isAuraIsland then
                _G.AuraKill = true
            else
                _G.AuraKill = false
            end
            ...
            if _G.AuraKill then
                ...
                for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                    ...
                    local dx = enemyRoot.Position.X - pos.X
                    local dz = enemyRoot.Position.Z - pos.Z
                    local dy = enemyRoot.Position.Y - pos.Y
                    local distSquared = dx*dx + dz*dz + dy*dy
  ...
  while wait(Sec) do
    pcall(function()
      if _G.Auto_Awakener then
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Awakener","Check")
        game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("Awakener","Awaken")
      end
    end)
  end
end)
    ...
    pcall(function()
      if _G.PortalUnLock then        
         ...
         elseif Attack.Pos(Man3Pos_Miti,8) then
           game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(-5072.08984375, 314.5412902832, -3151.1098632812))
         elseif Attack.Pos(HydraPos_Miti,8) then                    
           game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("requestEntrance",Vector3.new(5748.7587890625, 610.44982910156, -267.81704711914))
